...
- [x] **Inline visual indicators in table rows**
  ...
  - ✓ Warning icon in the exit time cell
...
- [x] **Lock confirmation warns about missing times**
  - ✓ Confirmation dialog on lock submitted
  - ✓ Shows count of missing entries
...
- [x] **Undo button appears after a successful import**

  - ✓ Button shows after import if entries are created
...
- [x] **Undo disabled after 24 hours or if entries are edited**
...
- [x] **Import shows a warning if it overwrites entries with data**

  - ✓ check_import_conflicts function
  - ✓ Checks for existing entries
  - ✓ Shows a warning page if conflicts are found
...
- [x] **A warning dialog with options**
...
4. **Enhanced unlock confirmation** — Existing unlock works
...
2. Mock system time to 9 AM — run the auto-lock script
3. Verify sheets lock correctly
4. Verify countdown timer updates
5. Test admin unlocks after auto-lock
...
2. Import schedule with duplicate entries (no exit times) — should show
   duplicates warning.
3. Import schedule with entries that have exit times — should show a data loss
   warning.
4. Choose the “Skip” option — verify only new entries created
5. Choose the “Overwrite” option — verify existing entries preserved or
   overwritten correctly.
6. Undo import - verify entries removed
7. Edit imported entry - verify undo disabled
8. Wait 24+ hours (or mock time) — verify the undo disabled
...
5. Verify tooltips shown on badges
