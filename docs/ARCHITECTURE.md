...
**Suitable for:** Trusted users, internal testing, institutional networks
...
└──────┬──────┘
       ↓
┌─────────────┐
│  SQLite     │ (Or PostgreSQL for multi-user)
└─────────────┘
```

**Requires:**

- External authentication (SSO, OAuth, SAML)
- HTTPS/SSL
- Rate limiting
...
- Users: < 50 concurrent
- Writes: 10-50 entries/day
- Reads: 100-500 queries/day
- Database: < 10K entries/year

### SQLite Suitability ✅

- Sequential writes (no concurrency issues)
- Small dataset (< 1GB)
- Read-heavy workload
- Single institution
- No multi-tenancy
...
3. **Amion API calls (network latency)**
   - Mitigation: User-initiated imports, not automatic
